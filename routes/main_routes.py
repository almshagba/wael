        ...
        for index, row in df.iterrows():
            try:
                ...
                if not school:
                    ...
                    if is_directorate and any(dept_name.replace('ال', '').replace(' ', '').strip() in dept.replace('ال', '').replace(' ', '').strip() for dept in DIRECTORATE_DEPARTMENTS):
                        school = School(name=dept_name, region="المديرية")
                        db.session.add(school)
                        db.session.flush()  # للحصول على ID
                        school_names[dept_name] = school
                        similar_schools[dept_name.replace('ال', '').replace(' ', '').strip()] = school
                    else:
